...
class StyleAdherenceLoss(nn.Module):
    def __init__(self, use_soft_min=True, lambda_softmin=10.0):
        """
        Initializes the StyleAdherenceLoss module.

        Args:
            use_soft_min (bool): If True, use soft minimization. If False, use hard minimization.
            lambda_softmin (float): Temperature parameter for the softmin approximation.
                                     Higher values make softmin closer to hard min.
                                     Ignored if use_soft_min is False.
        """
        super(StyleAdherenceLoss, self).__init__()
        self.use_soft_min = use_soft_min
        self.lambda_softmin = lambda_softmin

    def forward(self, X_pred, S, reduce=True):
        """
        Computes the style adherence loss.

        Args:
            X_pred (torch.Tensor): Predicted frames. Shape: (batch_size, T, feature_dim)
            S (torch.Tensor): Style clip frames. Shape: (batch_size, K, feature_dim)

        Returns:
            torch.Tensor: Scalar loss value.
        """
        # Ensure inputs are float tensors
        X_pred = X_pred.float()  # (B, T, F)
        S = S.float()            # (B, K, F)
        ...
        # X_pred_expanded: (B, T, 1, F)
        # S_expanded: (B, 1, K, F)
        X_pred_expanded = X_pred.unsqueeze(2)  # (B, T, 1, F)
        S_expanded = S.unsqueeze(1)            # (B, 1, K, F)

        # Compute squared Euclidean distances: (B, T, K)
        distances = torch.mean((X_pred_expanded - S_expanded) ** 2, dim=-1)  # (B, T, K)

        if self.use_soft_min:
            # Apply softmin across the K style frames for each T
            # To approximate the min, use softmax with negative distances
            softmin_weights = F.softmax(-self.lambda_softmin * distances, dim=-1)  # (B, T, K)

            # Compute the weighted sum of distances
            weighted_distances = softmin_weights * distances  # (B, T, K)
            loss_per_frame = torch.sum(weighted_distances, dim=-1)  # (B, T)

            # Aggregate the loss over batch and time
            if reduce:
                style_loss = torch.mean(loss_per_frame)
            else:
                style_loss = loss_per_frame

        else:
            # Hard minimization: Select the minimum distance for each frame
            min_distances, _ = torch.min(distances, dim=-1)  # (B, T)
            style_loss = torch.mean(min_distances)          # Scalar
...
def compute_loss_no_vert(args, is_starting_sample, shape_coef, motion_coef_gt, noise, target, prev_motion_coef, coef_stats,
                flame, end_idx=None, return_dict = False):
    """
    Compute the loss for the model without vertex position-based errors.
    Smoothness and velocity losses are computed in parameter space.
    
    Parameters:
        args: Configuration arguments.
        is_starting_sample (bool): Indicates if it's the starting sample.
        shape_coef (torch.Tensor): Shape coefficients.
        motion_coef_gt (torch.Tensor): Ground truth motion coefficients.
        noise (torch.Tensor): Predicted noise.
        target (torch.Tensor): Target tensor.
        prev_motion_coef (torch.Tensor): Previous motion coefficients.
        coef_stats (dict): Statistics for normalization.
        flame: Flame model for processing coefficients.
        end_idx (torch.Tensor, optional): End index for masking.
    
    Returns:
        Tuple of computed losses.
    """
    # Select the appropriate loss function based on the criterion
    ...
    # Initialize all possible loss components to None
    loss_noise = None
    loss_vel = None
    loss_smooth = None
    loss_head_vel = None
    ...
    if args.target == 'noise':
        # Compute loss between predicted noise and target noise
        loss_noise = criterion_func(noise, target[:, args.n_prev_motions:], reduction='none')
    elif args.target == 'sample':
        if is_starting_sample:
            # For starting samples, adjust the target
            target = target[:, args.n_prev_motions:]
        else:
            # Concatenate previous motion coefficients with ground truth
            motion_coef_gt = torch.cat([prev_motion_coef, motion_coef_gt], dim=1)
            if args.no_constrain_prev:
                # Concatenate previous motion coefficients with the adjusted target
                target = torch.cat([prev_motion_coef, target[:, args.n_prev_motions:]], dim=1)

        # Compute loss between ground truth motion coefficients and target
        loss_noise = criterion_func(motion_coef_gt, target, reduction='none')

        # If velocity or smoothness losses are enabled
        if args.l_vel > 0 or args.l_smooth > 0:
            # Extract parameters from ground truth and predicted coefficients
            coef_gt = motion_coef_gt
            coef_pred = target
            seq_len = target.shape[1]

            # Define parameter keys based on updated indices
            # Expression code is first 64 indices, pose has last 3 indices
            

            # Example: Assuming 'exp' is at indices 0-63 and 'pose' is at -3 to end
            # No need to handle 'shape' as vertex-based errors are removed

            # Convert parameters to tensors for loss computation
            # get the last three dimensions of coef_ft
            params_gt = {
                'exp': coef_gt[:, :, :-3],  # Shape: (batch_size, seq_len, 64)
                'pose': coef_gt[:, :, -3:]  # Shape: (batch_size, seq_len, 3)
            }
            params_pred = {
                'exp': coef_pred[:, :, :-3],  # Shape: (batch_size, seq_len, 64)
                'pose': coef_pred[:, :, -3:]  # Shape: (batch_size, seq_len, 3)
            }

            # Compute velocity loss: difference between consecutive parameters
            if args.l_vel > 0:
                vel_gt = params_gt['exp'][:, 1:] - params_gt['exp'][:, :-1]  # Shape: (batch_size, seq_len-1, 64)
                vel_pred = params_pred['exp'][:, 1:] - params_pred['exp'][:, :-1]  # Shape: (batch_size, seq_len-1, 64)
                loss_vel_exp = criterion_func(vel_gt, vel_pred, reduction='none')  # Shape: (batch_size, seq_len-1, 64)

                vel_gt_pose = params_gt['pose'][:, 1:] - params_gt['pose'][:, :-1]  # Shape: (batch_size, seq_len-1, 3)
                vel_pred_pose = params_pred['pose'][:, 1:] - params_pred['pose'][:, :-1]  # Shape: (batch_size, seq_len-1, 3)
                loss_vel_pose = criterion_func(vel_gt_pose, vel_pred_pose, reduction='none')  # Shape: (batch_size, seq_len-1, 3)

                # Combine velocity losses for all parameters
                loss_vel = loss_vel_exp.mean(dim=-1) + loss_vel_pose.mean(dim=-1)  # Shape: (batch_size, seq_len-1)

            # Compute smoothness loss: difference between consecutive velocities
            if args.l_smooth > 0:
                if args.l_vel > 0:
                    # Already have velocity predictions if args.l_vel > 0
                    vel_pred_exp = vel_pred  # Shape: (batch_size, seq_len-1, 64)
                    vel_pred_pose = vel_pred_pose  # Shape: (batch_size, seq_len-1, 3)
                else:
                    # Compute velocities if not already computed
                    vel_pred_exp = params_pred['exp'][:, 1:] - params_pred['exp'][:, :-1]
                    vel_pred_pose = params_pred['pose'][:, 1:] - params_pred['pose'][:, :-1]

                # Compute smoothness as the difference between consecutive velocities
                smooth_pred_exp = vel_pred_exp[:, 1:] - vel_pred_exp[:, :-1]  # Shape: (batch_size, seq_len-2, 64)
                smooth_pred_pose = vel_pred_pose[:, 1:] - vel_pred_pose[:, :-1]  # Shape: (batch_size, seq_len-2, 3)

                # Compute smoothness loss
                loss_smooth_exp = criterion_func(smooth_pred_exp, torch.zeros_like(smooth_pred_exp), reduction='none')  # Shape: (batch_size, seq_len-2, 64)
                loss_smooth_pose = criterion_func(smooth_pred_pose, torch.zeros_like(smooth_pred_pose), reduction='none')  # Shape: (batch_size, seq_len-2, 3)

                # Combine smoothness losses for all parameters
                loss_smooth = loss_smooth_exp.mean(dim=-1) + loss_smooth_pose.mean(dim=-1)  # Shape: (batch_size, seq_len-2)

        # Head pose losses in parameter space
        if not args.no_head_pose:
            if args.rot_repr == 'aa':
                # Assuming neck pose parameters are the last 3 indices
                head_pose_gt = motion_coef_gt[:, :, -3:]  # Shape: (batch_size, seq_len, 3)
                head_pose_pred = target[:, :, -3:]  # Shape: (batch_size, seq_len, 3)
            else:
                raise ValueError(f'Unknown rotation representation {args.rot_repr}!')
            loss_head_angle = criterion_func(head_pose_gt, head_pose_pred, reduction='none')  # Shape: (batch_size, seq_len, 3)
            # Compute head pose velocity loss
            if args.l_head_vel > 0:
                head_vel_gt = head_pose_gt[:, 1:] - head_pose_gt[:, :-1]  # Shape: (batch_size, seq_len-1, 3)
                head_vel_pred = head_pose_pred[:, 1:] - head_pose_pred[:, :-1]  # Shape: (batch_size, seq_len-1, 3)
                loss_head_vel = criterion_func(head_vel_gt, head_vel_pred, reduction='none')  # Shape: (batch_size, seq_len-1, 3)
                loss_head_vel = loss_head_vel.mean(dim=-1)  # Shape: (batch_size, seq_len-1)

            # Compute head pose smoothness loss
            if args.l_head_smooth > 0:
                # Compute velocity differences if not already computed
                if args.l_head_vel > 0:
                    head_vel_pred = head_vel_pred  # Already computed
                else:
                    head_vel_pred = head_pose_pred[:, 1:] - head_pose_pred[:, :-1]

                head_smooth_pred = head_vel_pred[:, 1:] - head_vel_pred[:, :-1]  # Shape: (batch_size, seq_len-2, 3)
                loss_head_smooth = criterion_func(head_smooth_pred, torch.zeros_like(head_smooth_pred), reduction='none')  # Shape: (batch_size, seq_len-2, 3)
                loss_head_smooth = loss_head_smooth.mean(dim=-1)  # Shape: (batch_size, seq_len-2)

            # Compute head pose transition losses if applicable
            if not is_starting_sample and args.l_head_trans > 0:
                # Version 2: Constrain only the predicted current motions (x_{0} ~ x_{2})
                # Adjusted to reflect new pose indices
                head_pose_trans = torch.cat([
                    head_pose_gt[:, args.n_prev_motions - 3:args.n_prev_motions],
                    head_pose_pred[:, args.n_prev_motions:args.n_prev_motions + 3]
                ], dim=1)  # Shape: (batch_size, 3 + 3, 3)

                head_vel_pred_trans = head_pose_trans[:, 1:] - head_pose_trans[:, :-1]  # Shape: (batch_size, 5, 3)
                head_accel_pred_trans = head_vel_pred_trans[:, 1:] - head_vel_pred_trans[:, :-1]  # Shape: (batch_size, 4, 3)

                # Constrain velocities between specific frames
                loss_head_trans_vel = criterion_func(
                    head_vel_pred_trans[:, 2:4],
                    head_vel_pred_trans[:, 1:3],
                    reduction='none'
                )  # Shape: (batch_size, 2, 3)
                loss_head_trans_vel = loss_head_trans_vel.mean(dim=-1).mean(dim=-1)  # Shape: (batch_size, 2)

                # Constrain accelerations between specific frames
                loss_head_trans_accel = criterion_func(
                    head_accel_pred_trans[:, 1:],
                    head_accel_pred_trans[:, :-1],
                    reduction='none'
                )  # Shape: (batch_size, 3, 3)
                loss_head_trans_accel = loss_head_trans_accel.mean(dim=-1).mean(dim=-1)  # Shape: (batch_size, 3)

                # Combine transition losses
                loss_head_trans = loss_head_trans_vel + loss_head_trans_accel  # Shape: (batch_size, 2) + (batch_size, 3) -> (batch_size, 5)
        
    else:
        raise ValueError(f'Unknown diffusion target: {args.target}')

    # Create the mask for valid frames
    if end_idx is None:
    ...
    # Adjust mask based on whether it's a sample and if previous constraints are applied
    ...
    # Apply the mask and compute the mean of the losses
    loss_noise = loss_noise[mask].mean() if loss_noise is not None else None

    if loss_vel is not None:
        # Shift the mask for velocity loss
        vel_mask = mask[:, 1:]
        loss_vel = loss_vel[vel_mask].mean() if torch.numel(loss_vel[vel_mask]) > 0 else None

    if loss_smooth is not None:
        # Shift the mask for smoothness loss
        smooth_mask = mask[:, 2:]
        loss_smooth = loss_smooth[smooth_mask].mean() if torch.numel(loss_smooth[smooth_mask]) > 0 else None

    if loss_head_angle is not None:
        loss_head_angle = loss_head_angle[mask].mean() if torch.numel(loss_head_angle[mask]) > 0 else None


    if loss_head_vel is not None:
        # Shift the mask for head velocity loss
        head_vel_mask = mask[:, 1:]
        loss_head_vel = loss_head_vel[head_vel_mask].mean() if torch.numel(loss_head_vel[head_vel_mask]) > 0 else None

    if loss_head_smooth is not None:
        # Shift the mask for head smoothness loss
        head_smooth_mask = mask[:, 2:]
        loss_head_smooth = loss_head_smooth[head_smooth_mask].mean() if torch.numel(loss_head_smooth[head_smooth_mask]) > 0 else None

    if loss_head_trans is not None:
        # No additional masking needed for head_trans as it's already computed based on transitions
        loss_head_trans = loss_head_trans.mean() if torch.numel(loss_head_trans) > 0 else None
    if return_dict == False:
        return loss_noise/2, loss_vel/2, loss_smooth/2, loss_head_angle/2, loss_head_vel/2, loss_head_smooth/2, loss_head_trans
        ...
        if loss_smooth is None:
            loss_smooth = 0
        if loss_head_angle is None:
            loss_head_angle = 0
        if loss_head_vel is None:
            loss_head_vel = 0
        if loss_head_smooth is None:
            loss_head_smooth = 0
        out_loss_dict = {"noise": loss_noise/2, 
                    "vel": loss_vel/2, 
                    "smooth": loss_smooth/2, 
                    "head_angle": loss_head_angle/2, 
                    "head_vel": loss_head_vel/2, 
                    "head_smooth": loss_head_smooth/2,
                    "head_trans": loss_head_trans}
        return out_loss_dict
...
def compute_loss(args, is_starting_sample, shape_coef, motion_coef_gt, noise, target, prev_motion_coef, coef_stats,
    ...
    if return_dict == False:
        return loss_noise/2, loss_vert/2, loss_vel/2, loss_smooth/2, loss_head_angle/2, loss_head_vel/2, loss_head_smooth/2, \
            loss_head_trans
    else:
        out_loss_dict = {"noise": loss_noise/2, 
                    "vert": loss_vert/2, 
...
def compute_loss_espnet(args, is_starting_sample, shape_coef, motion_coef_gt, noise, target, prev_motion_coef, coef_stats,
                 gt_vertices, seq_vertices, end_idx=None, return_dict=False):
    if args.criterion.lower() == 'l2':
        criterion_func = F.mse_loss
    elif args.criterion.lower() == 'l1':
        criterion_func = F.l1_loss
    ...
    if args.target == 'noise':
        ...
        if args.l_vert > 0 or args.l_vel > 0:
            ...
            if args.rot_repr == 'aa':
                verts_gt = gt_vertices
                verts_pred = seq_vertices
            else:
                raise ValueError(f'Unknown rotation representation {args.rot_repr}!')
            # verts_gt = verts_gt.view(-1, seq_len, 5023, 3) # don't need this 
            # verts_pred = verts_pred.view(-1, seq_len, 5023, 3) # don't need this 
        ...
        if not args.no_head_pose:
            if args.rot_repr == 'aa':
                head_pose_gt = motion_coef_gt[:, :, -3:]
                head_pose_pred = target[:, :, -3:]
            else:
                raise ValueError(f'Unknown rotation representation {args.rot_repr}!')
...
def truncate_motion_coef_and_audio(audio, motion_coef, n_motions, audio_unit=640, pad_mode='zero', expression_code_size=50):
    batch_size = audio.shape[0]
    end_idx = torch.randint(1, n_motions, (batch_size,), device=audio.device)
    audio_end_idx = (end_idx * audio_unit).long()
    # mask = torch.arange(n_motions, device=audio.device).expand(batch_size, -1) < end_idx.unsqueeze(1)
    ...
    coef_dict = {'exp': motion_coef[..., :expression_code_size], 'pose_any': motion_coef[..., expression_code_size:]}
